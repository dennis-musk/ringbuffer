 ...
 * the data in the buffer will keep there until they be read out. If the buffer
 * is full, the data could not be put into any more, until a read action free
 * some space.  
 */
 ...
 */
	...
	ring_buf->data = kmalloc(size, gfp_mask);
	if (!ring_buf->data) {
		kfree(ring_buf);
		return ERR_PTR(-ENOMEM);
	}
...
/**
 *  ringbuffer_toio - get the data from io memory, and put them into ringbuffer
 *  @ring_buf: the ringbuffer to be used.
		...
		unsigned long len)
{
	...
	len = min(len, (unsigned long)(ring_buf->write_pos - ring_buf->read_pos));
